...
[![Download DMG](https://img.shields.io/badge/下载-v0.9.9%20DMG%20(Apple%20芯片)-2E7D32?style=flat-square&logo=apple&logoColor=white)](https://github.com/YangChen-cn/Ascend/releases/download/v0.9.9/Ascend-v0.9.9-arm64.dmg)
...
[📥 **直接下载 v0.9.9 (Apple 芯片 DMG)**](https://github.com/YangChen-cn/Ascend/releases/download/v0.9.9/Ascend-v0.9.9-arm64.dmg) · [📖 **查看更新日志 (Release Notes)**](https://github.com/YangChen-cn/Ascend/releases/tag/v0.9.9)
...
知境录坚守「真实能力沉淀」与「无压沉浸研习」，确立了六大底层产品心法与设计权衡：

1. **自动化 vs 可信度 ·「低境界偏体验，高境界偏证据」**
   - 若完全自动，看了文档就标“掌握”，体验虽轻但不可信；若完全验证，每学一个点都要做题，虽可信但极其烦琐。
   - 知境录的权衡：**真实产物（弱证据）$\rightarrow$ 自动成长至通晓；主动验证（强证据）$\rightarrow$ 突破融会；真实实作（更强证据）$\rightarrow$ 晋阶化用与通达。**
2. **学习成长 vs 考试负担 ·「题目是加速器与校准器，不是过路费」**
   - 默认主干路径：正常阅读文档与编写代码，后台隐式理解，境界自然增长，无需多余操作。
   - 答题完全自愿：点击「主动研习」答 1～3 题即可校准能力并加速成长；接近融会时自然转为「主动印证」。
3. **AI 赋能 vs API 成本 ·「一次请求多重产出，本地运转零 AI 开销」**
   - 拒绝单次微小操作反复调用 API：单次学习分析同时提炼 Evidence、概念纳新、关系推导、下一境建议与隐藏题包。
   - 题包顺带生成并本地持久化，后续研习 0 AI 调用；**默认晨昏温故彻底 0 AI 调用**，本地负责 FSRS 推演、计分与判题。
4. **精密模型 vs 极简呈现 ·「内部可以复杂，外部必须极简」**
   - 内部维系五维评分、贝叶斯后验、BKT 步长、FSRS 与 Production Gates；外部仅呈现「我现在在哪个境界？为什么是这个境界？下一步怎么提升？」，杜绝数字轰炸。
5. **获得感 vs 杜绝刷分 ·「XP 是能力增长的结果，而非行为点击的奖励」**
   - 拒绝形式主义打卡加分；**XP 仅由掌握度真实正向净增量结算**，重复刷题无新能力信息则严格 0 XP。
6. **“记得” vs “会用” ·「四维解耦，绝不互相冒充」**
   - **Artifact**（接触与学习事实） $\neq$ **Assessment**（概念会不会） $\neq$ **Review**（能不能想起来） $\neq$ **Production**（能否真实独立落地），四者各司其职、互为印证。
...
### 1. 直接下载 DMG 镜像（推荐）
- 前往 [GitHub Releases](https://github.com/YangChen-cn/Ascend/releases/latest) 下载最新版本的 `Ascend-v0.9.9-arm64.dmg`。
